...
开发工作区版本：`0.9.0.dev1`；正式部署目录仍是已验证的 `0.8.0.dev1` 基线，Git 提交 `fc0b138`、标签 `v0.8.0-dev.1`。`0.9.0.dev1` 新增版本化任务信封与结构化结果回写、CLI/Codex skill/MCP 桥梁、fast 整理 + reasoning 审阅、多模式联网查证和受控改进提案；完成完整验收、数据库备份和部署同步前，不把它标记为正式服务版本。

2026-08-09 工作区完整验收：`183 passed`，总覆盖率 `81.44%`；Ruff、Pyright、前端语法、skill 校验和 `git diff --check` 均通过。
