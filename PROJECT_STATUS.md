...
开发工作区与正式部署版本：`0.9.0.dev2`。2026-08-09 部署前已创建并校验 SQLite 一致性备份与旧版应用源码快照，正式数据库已从 schema 3 迁移到 schema 5。该版本新增版本化任务信封与结构化结果回写、CLI/Codex skill/MCP 桥梁、fast 整理 + reasoning 审阅、多模式联网查证和受控改进提案；`dev2` 另确保本机 CLI 桥梁不经系统代理访问回环地址。

2026-08-09 工作区完整验收：`184 passed`，总覆盖率 `81%`；Ruff、Pyright、前端语法、skill 校验和 `git diff --check` 均通过。
...
- MCP 只读工具服务：`backend/mcp_server.py` 暴露资料、学习、教练、科研、任务信封、改进提案、Zotero、运维和审计工具，供 Codex、Cline、CLI 等调用；工具只读、不读取密钥。
...
- 受控联网检索：支持 `auto/on/off`；明确命令和 `on` 直接检索，`auto` 仅在时效/查证型问题触发，并阻止疑似本机路径、邮箱、令牌和手机号外发。`ddgs` 最多返回 5 条公开网页摘要，与本地资料分开标注并视为不可信数据。
- 多模型协作：`POST /api/collaboration/run` 先用 fast 角色整理事实、冲突和缺口，再用 reasoning 角色独立核对、反方审阅并生成最终答案；两阶段分别写入 `model_calls`，共享运行 ID，用户仍控制两个角色的服务商和模型。
- 跨工具桥梁：`backend.cli`、Codex skill、MCP 与网页共享 `nexus.task-envelope` v1；结果回写保存摘要、引用、产物、测试和 commit，过期信封返回 409，不同步完整对话或登录态。
- 受控改进提案：系统从近 30 天模型失败和重复审计错误生成去重提案；只有本机显式批准后才创建隔离 Agent 实验任务，不自动修改、合并或部署。
...
1. [x] `nexus.task-envelope` v1、`backend.cli`、Codex skill 与 MCP 已共享任务 ID、修订、约束、哈希和结构化结果；资料引用可回写，自动附加资料 ID 的任务创建 UI 仍待完善。
2. [x] 模型路由表已接入：按任务持久化路由规则（auto / 固定角色 / 低预算优先），复杂度启发式选择深度推理或快速任务，显式角色始终优先；隐私与质量维度的自动评估仍待完善。
3. [x] `model_calls` 已记录 provider、model、角色、来源、耗时、成本估算和错误码；fast 整理 + reasoning 独立审阅已接入，自动选择第三个独立核查服务商仍待完善。
4. 保持对话执行器可替换，避免把长期知识绑定到某个聊天窗口或供应商。

### P1：科学学习与反讨好工作流

1. [x] 统一 AI 对话提示词已纳入本地资料、证据分级、反例/边界条件和不确定性要求；隐私感知的 `auto/on/off` 联网查证和多模型科学审阅提示词已接入。
...
1. [x] 已从近 30 天模型失败率和重复审计错误发现非敏感改进信号并生成去重提案；用户修正与检索质量指标仍待接入。
2. [x] 人工显式批准后可把提案转换为隔离 Agent 实验任务；自动生成实验补丁和评估报告由执行器完成，尚未自动触发。
3. 正式部署仍必须人工批准、备份、完整测试、观察和可回滚；当前系统不会自动合并或发布。
